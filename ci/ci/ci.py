...
@routes.get('/envoy_configs')
@auth.authenticated_developers_only()
async def get_envoy_configs(request: web.Request, _) -> web.Response:
    db = request.app[AppKeys.DB]
    namespaces = list(set([DEFAULT_NAMESPACE, 'default']))
    return json_response(await generate_envoy_configs(db, namespaces))
...
async def update_envoy_configs(db: Database, k8s_client):
    ...
    envoy_configs = await generate_envoy_configs(db, live_namespaces)
    for proxy, (cds_config, rds_config) in envoy_configs.items():
...
async def generate_envoy_configs(db: Database, namespaces: List[str]) -> Dict[str, Tuple[dict, dict]]:
    namespace_arg_list = "(" + ",".join('%s' for _ in namespaces) + ")"
...
GROUP BY active_namespaces.namespace""",
    ...
    return {
        proxy: (
            create_cds_response(default_services, services_per_namespace, proxy),
            create_rds_response(default_services, services_per_namespace, proxy),
        )
